...
create table cart_object (id bigint not null auto_increment, quantity integer, cart_id bigint, item_id bigint, primary key (id)) engine=InnoDB
...
alter table cart_object add constraint FK6cb3gydx56dv05qiq2nsdb37k foreign key (cart_id) references cart (id)
alter table cart_object add constraint FKtlg4nqgi1amfo4yddntbbnhdb foreign key (item_id) references item (id)
